...
- (IBAction)createNewShakeList:(id)sender {
    ...
    if ([self.shakeListTitleTextField.text isEqualToString:@""]) {
        ...
        [shakeListData setObject:self.phraseArray
                          forKey:@"phrases"];
        [shakeListData setValue:[NSNumber numberWithLong:TIME_STAMP]
                         forKey:@"created-at"];
...
- (void)setUploadingStatus:(BOOL) flag {
    ...
    self.backBarButtonItem.enabled = flag;
}

// Push this viewcontroller after save the data.
- (void)pushViewController {
    
    UIViewController *controller = [self.storyboard instantiateViewControllerWithIdentifier:MY_SHAKELIST_VIEWID];
...
- (void) animateTextField: (UITextField*) textField up: (BOOL) up {
    ...
    if (index > 7) index = 7;
    const int movementDistance = 60 + index*25; // tweak as needed
